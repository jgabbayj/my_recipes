...
import com.morsel.recipes.BuildConfig
...
object GeminiParser {
    ...
    suspend fun parseFromUrl(url: String, apiKey: String): ParseResult = withContext(Dispatchers.IO) {
        val finalApiKey = BuildConfig.GEMINI_API_KEY
        ...
        if (mockKey != null || finalApiKey.isEmpty()) {
        ...
        try {
            ...
            val result = parseFromText(contentToParse, finalApiKey)
    ...
    suspend fun parseFromText(text: String, apiKey: String): ParseResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val responseText = callGeminiApi(prompt, finalApiKey)
